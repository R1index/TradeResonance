...
def index():
    ...
    if tab == "cities":
        ...
        for city, product in prod_rows:
            produces_map.setdefault(city, []).append(product)
        for k in produces_map:
            produces_map[k] = sorted(set(produces_map[k]))

        # avg prices per city
        rows = (
            db.session.query(Entry.city, func.avg(Entry.price))
            .group_by(Entry.city)
            .order_by(Entry.city.asc())
            .all()
        )
        cities = [
            {"city": c, "avg_price": float(avg or 0), "produces": produces_map.get(c, [])}
            for c, avg in rows
        ]
        return render_template("cities.html", cities=cities, t=t, lang=lang)
