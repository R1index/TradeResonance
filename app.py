...
def new_entry():
    ...
    cities_list = [c for (c,) in db.session.query(Entry.city).distinct().order_by(Entry.city.asc()).all()]
    products_list = [p for (p,) in db.session.query(Entry.product).distinct().order_by(Entry.product.asc()).all()]
    return render_template("entry_form.html", e=None, title=t("new_entry"), cities_list=cities_list, products_list=products_list)
...
def edit_entry(entry_id):
    ...
    cities_list = [c for (c,) in db.session.query(Entry.city).distinct().order_by(Entry.city.asc()).all()]
    products_list = [p for (p,) in db.session.query(Entry.product).distinct().order_by(Entry.product.asc()).all()]
    return render_template("entry_form.html", e=e, title=t("edit_entry"), cities_list=cities_list, products_list=products_list)
